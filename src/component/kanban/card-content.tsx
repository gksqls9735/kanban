  ...
  return (
    <>
      <div className="card-header" style={{ display: 'flex', justifyContent: 'space-between' }}>
        <div className="card-current-section" style={{ position: 'relative' }}>{sectionName}</div>
        <div style={{ visibility: isHovered ? 'visible' : 'hidden', border: '1px solid #E4E8EE', borderRadius: 4, display: 'flex', cursor: 'pointer' }}>
          {/** 수정 */}
          <div style={{ borderRight: '1px solid #E4E8EE', width: 24, height: 24 }}>
          ...
          {/** 더보기 */}
          <div ref={wrapperRef} style={{ width: 24, height: 24 }} onClick={toggle}>
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960" fill="#8D99A8" style={{ padding: 5 }}>
        ...
        {isOpen && (
          ...
          >
            {/* 드롭다운 메뉴 항목들 */}
            <div style={{ padding: '8px 12px', cursor: 'pointer', color: '#0F1B2A', display: 'flex', gap: '10px', alignItems: 'center' }} onClick={handleCopy}>
              {/* 작업 복사 아이콘 (예시) */}
            ...
            <div style={{ padding: '8px 12px', cursor: 'pointer', color: '#0F1B2A', display: 'flex', gap: '10px', alignItems: 'center' }} onClick={handleDelete}>
              {/* 작업 삭제 아이콘 (예시) */}
...
const areEqual = (prevProps: Readonly<{ task: Task; sectionName: string; isHovered: boolean; }>, nextProps: Readonly<{ task: Task; sectionName: string; isHovered: boolean; }>): boolean => {
